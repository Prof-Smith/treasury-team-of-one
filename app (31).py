...
if page == 'Historical Baseline':
    ...
    left,right=st.columns([2.25,1],gap='large')
    with left: st.plotly_chart(fan(result,'Friday baseline vs. current risk',friday),use_container_width=True)
    with right:
        a,b=st.columns(2); a.metric('VaR 95%',money(result['var'])); b.metric('CVaR 95%',money(result['cvar']))
        c,d=st.columns(2); c.metric('P(Below)',f"{result['pmin']:.1%}"); d.metric('P(Negative)',f"{result['pneg']:.1%}")
        if stage<4:
            if st.button('Release next event →',type='primary',use_container_width=True): st.session_state.storm+=1; st.rerun()
        elif st.button('Reveal liquidity dilemma →',type='primary',use_container_width=True): st.session_state.page=10; st.rerun()
    ...
    with right:
        ...
        if st.button('Compare responses →',type='primary',use_container_width=True): st.session_state.page=10; st.rerun()
    ...
    with right:
        ...
        if st.button('Create CFO brief →',type='primary',use_container_width=True): st.session_state.page=10; st.rerun()
