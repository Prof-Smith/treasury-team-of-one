...
.clockbar{{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:.7rem;background:#FFFFFF;border:1px solid #D5E1E8;border-radius:11px;padding:.42rem .68rem;margin:0 0 .45rem;box-shadow:0 2px 8px rgba(8,31,51,.05)}}
.clocktime{{font-size:1.02rem;font-weight:900;color:#081F33;white-space:nowrap}}.clocklabel{{font-size:.69rem;letter-spacing:.08em;text-transform:uppercase;color:#687C8E;font-weight:800}}.clocktrack{{height:7px;background:#E5EDF2;border-radius:999px;overflow:hidden}}.clockfill{{height:100%;background:linear-gradient(90deg,#00BAC6,#F4AB00,#D63F42);border-radius:999px}}.clockremain{{font-size:.78rem;font-weight:800;color:#D63F42;white-space:nowrap}}
[data-testid="stSidebar"] .sideclock{{border:1px solid rgba(255,255,255,.18);border-radius:11px;padding:.55rem .65rem;background:rgba(255,255,255,.06);margin:.35rem 0 .7rem}}[data-testid="stSidebar"] .sideclock .big{{font-size:1.35rem;font-weight:900;color:#FFFFFF}}[data-testid="stSidebar"] .sideclock .small{{font-size:.72rem;color:#BFD2DE}}
...
BRIEFING_SCHEDULE = {
    'Historical Baseline': ('Friday 4:20 PM', 0, 'Baseline preparation'),
    'Friday Snapshot': ('Friday 4:30 PM', 0, 'Risk baseline approved'),
    'Organization Map': ('Monday 8:00 AM', 0, 'Operating context'),
    'Monday Inbox': ('Monday 8:03 AM', 3, 'New evidence arrives'),
    'Agent Operations': ('Monday 8:08 AM', 8, 'Agents process submissions'),
    'Human Review': ('Monday 8:15 AM', 15, 'Material judgments'),
    'The Storm': ('Monday 8:28 AM', 28, 'Forecast assumptions deteriorate'),
    'Dilemma': ('Monday 8:42 AM', 42, 'Liquidity decision required'),
    'Response Studio': ('Monday 8:55 AM', 55, 'Responses tested'),
    'CFO Brief': ('Monday 9:25 AM', 85, 'Decision brief ready'),
    'Audit & Downloads': ('Monday 9:30 AM', 90, 'Briefing and audit trail'),
}

def briefing_clock(current_page):
    display_time, elapsed, phase = BRIEFING_SCHEDULE[current_page]
    remaining = max(0, 90 - elapsed)
    width = min(100, round(elapsed / 90 * 100))
    remaining_text = 'Before Monday' if current_page in ('Historical Baseline','Friday Snapshot') else ('Briefing time' if remaining == 0 else f'{remaining} min to CFO brief')
    st.markdown(f'<div class="clockbar"><div><div class="clocklabel">Briefing clock</div><div class="clocktime">{display_time}</div></div><div><div class="clocklabel">{phase}</div><div class="clocktrack"><div class="clockfill" style="width:{width}%"></div></div></div><div class="clockremain">{remaining_text}</div></div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    clock_time, clock_elapsed, clock_phase = BRIEFING_SCHEDULE[page]
    clock_remaining = 'Before Monday' if page in ('Historical Baseline','Friday Snapshot') else ('Briefing time' if clock_elapsed >= 90 else f'{90-clock_elapsed} minutes remain')
    st.markdown(f'<div class="sideclock"><div class="small">BRIEFING CLOCK</div><div class="big">{clock_time}</div><div class="small">{clock_phase}<br>{clock_remaining}</div></div>', unsafe_allow_html=True)
    st.markdown('---')
