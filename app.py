...
if page == 'Historical Baseline':
    ...
    with right:
        ...
        st.markdown(f'''<div class="panel"><div class="eyebrow">EVIDENCE CAUSING THIS MOVE</div><div class="row"><b>Email</b><span class="grow"></span><span>{ev['email']}</span></div><div class="row"><b>Attachment</b><span class="grow"></span><span>{ev['attachment']}</span></div><p><b>Observed:</b> {ev['raw']}</p><p><b>Model update:</b> {ev['model_change']}</p><p><b>Cash-path effect:</b> {ev['cash_effect']}</p><p><b>Risk-band effect:</b> {ev['uncertainty_effect']}</p></div>''',unsafe_allow_html=True)
        with st.expander('Show stage-to-stage metric bridge'):
            bridge = pd.DataFrame([
