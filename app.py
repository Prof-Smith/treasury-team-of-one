...
.titlehero{{position:relative;min-height:520px;overflow:hidden;display:grid;grid-template-columns:1.35fr .9fr;align-items:center;gap:1.6rem;background:linear-gradient(135deg,#061C2D 0%,#0B3E59 58%,#086B78 100%);color:white;border-radius:24px;padding:2.7rem 3rem;box-shadow:0 20px 48px rgba(8,31,51,.24)}}
.titlehero:before{{content:'';position:absolute;inset:0;background-image:linear-gradient(rgba(255,255,255,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.045) 1px,transparent 1px);background-size:38px 38px;mask-image:linear-gradient(to right,transparent 8%,black 70%);pointer-events:none}}
.titlecopy{{position:relative;z-index:2;text-align:left}}.titlehero h1{{font-size:3.5rem;line-height:.98;margin:.45rem 0 .65rem;max-width:760px}}.titlehero h2{{font-size:1.42rem;font-weight:600;color:#D9EDF2;margin:0 0 .8rem}}.titlehero p{{color:#C5DCE5;font-size:1.03rem;line-height:1.45;margin:.3rem 0 1rem}}.titlemeta{{font-size:.83rem;letter-spacing:.04em;color:#C6DAE3;margin-top:1.35rem}}.casebadge{{display:inline-block;border:1px solid rgba(255,255,255,.3);background:rgba(255,255,255,.09);border-radius:999px;padding:.35rem .68rem;margin:.16rem;font-size:.76rem;font-weight:800}}
.agentvisual{{position:relative;z-index:2;height:330px;display:flex;align-items:center;justify-content:center}}.agentcore{{position:relative;width:138px;height:138px;border-radius:50%;display:flex;align-items:center;justify-content:center;text-align:center;padding:1rem;background:radial-gradient(circle at 35% 30%,#42E4ED,#008B99 62%,#005A67);box-shadow:0 0 0 14px rgba(66,228,237,.08),0 0 0 30px rgba(66,228,237,.045),0 18px 40px rgba(0,0,0,.28);font-size:.8rem;font-weight:900;letter-spacing:.06em;animation:corePulse 2.8s ease-in-out infinite}}.agentorbit{{position:absolute;width:292px;height:292px;border:1px solid rgba(129,235,241,.35);border-radius:50%;animation:orbitSpin 18s linear infinite}}.agentorbit:after{{content:'';position:absolute;inset:35px;border:1px dashed rgba(255,255,255,.22);border-radius:50%}}.agentnode{{position:absolute;width:90px;padding:.42rem .35rem;border-radius:10px;background:rgba(4,29,45,.9);border:1px solid rgba(104,226,234,.42);font-size:.64rem;text-align:center;font-weight:800;box-shadow:0 7px 18px rgba(0,0,0,.2);animation:counterSpin 18s linear infinite;transform-origin:center}}.n1{{top:0;left:101px}}.n2{{top:64px;right:-18px}}.n3{{bottom:54px;right:-12px}}.n4{{bottom:-4px;left:101px}}.n5{{bottom:54px;left:-12px}}.n6{{top:64px;left:-18px}}.signal{{position:absolute;width:8px;height:8px;border-radius:50%;background:#FFE08A;box-shadow:0 0 15px #FFE08A;top:18px;left:58px;animation:signalPulse 1.7s ease-in-out infinite}}@keyframes corePulse{{0%,100%{{transform:scale(1)}}50%{{transform:scale(1.035)}}}}@keyframes orbitSpin{{to{{transform:rotate(360deg)}}}}@keyframes counterSpin{{to{{transform:rotate(-360deg)}}}}@keyframes signalPulse{{0%,100%{{opacity:.35}}50%{{opacity:1}}}}@media (prefers-reduced-motion:reduce){{.agentcore,.agentorbit,.agentnode,.signal{{animation:none!important}}}}
.titleactions div[data-testid="stButton"] button{{min-height:48px;font-size:1rem}}.actflow{{display:grid;grid-template-columns:repeat(4,1fr);gap:.55rem;margin:.65rem 0}}.actnode{{background:white;border:1px solid #D5E1E8;border-radius:12px;padding:.65rem;text-align:center;font-size:.8rem;font-weight:800;color:#17324A}}
@media (max-width:1100px){{.titlehero{{grid-template-columns:1fr;min-height:auto;padding:2rem}}.agentvisual{{display:none}}.titlehero h1{{font-size:2.8rem}}}}
...
if page=='Title':
    st.markdown('''<div class="titlehero"><div class="titlecopy"><div class="eyebrow">TAMPA BAY AFP | SEPTEMBER 11, 2026</div><h1>The Treasury Team of One</h1><h2>Building AI-Powered Financial Solutions</h2><p>From fragmented evidence to a governed liquidity decision</p><div><span class="casebadge">6 Entities</span><span class="casebadge">3 Currencies</span><span class="casebadge">8 Messages</span><span class="casebadge">10 Attachments</span><span class="casebadge">1 CFO Deadline</span></div><div class="titlemeta">Zachary A. Smith, Ph.D.<br>Associate Professor of Economics and Finance | Saint Leo University</div></div><div class="agentvisual"><div class="agentorbit"><div class="agentnode n1">INBOX<br>AGENT</div><div class="agentnode n2">VERSION<br>AGENT</div><div class="agentnode n3">DOCUMENT<br>AGENT</div><div class="agentnode n4">MAPPING<br>AGENT</div><div class="agentnode n5">RISK<br>AGENT</div><div class="agentnode n6">CONTROL<br>AGENT</div></div><div class="agentcore">TREASURY<br>DECISION<br>ENGINE<span class="signal"></span></div></div></div>''',unsafe_allow_html=True)
