...
pages=['Historical Upload','Friday Snapshot','Monday Inbox','Agent Operations','Human Review','The Storm','Dilemma','Response Studio','CFO Brief','Audit & Downloads']
with st.sidebar:
 st.markdown('## ◈ Treasury Agent Operations'); st.caption('One upload. Monday agents take over.')
...
if page=='Historical Upload':
 hero('T−2 | BEFORE MONDAY','Upload historical forecast performance','This is the only manual upload. The Risk Agent learns prior forecast variance before current submissions arrive.')
 l,r=st.columns([1.2,1],gap='large')
 with l:
  up=st.file_uploader('Historical forecast-error file',type=['csv','xlsx','xls']); use=st.button('Use bundled demonstration history',use_container_width=True)
  source=up if up else (SAMPLE if use else None)
  if source:
   try: st.session_state.hist=load_history(source); st.success(f"Historical file accepted: {len(st.session_state.hist)} complete observations.")
   except Exception as e: st.error(str(e))
 with r:
  st.markdown('<div class="panel"><div class="eyebrow">REQUIRED FIELDS</div><div class="row"><b>date</b><span class="grow"></span><span>observation date</span></div><div class="row"><b>opening_error</b><span class="grow"></span><span>opening cash variance</span></div><div class="row"><b>receipt_error</b><span class="grow"></span><span>receipt variance</span></div><div class="row"><b>disbursement_error</b><span class="grow"></span><span>payment variance</span></div><br><div class="muted">At least 60 complete observations. No Monday files are uploaded manually.</div></div>',unsafe_allow_html=True)
 if st.session_state.hist is not None:
  a,b,c=st.columns(3); h=st.session_state.hist; a.metric('Observations',len(h)); b.metric('Beginning',h.date.min().strftime('%b %Y')); c.metric('Ending',h.date.max().strftime('%b %Y'))
  if st.button('Train Friday risk baseline →',type='primary',use_container_width=True): st.session_state.page=1; st.rerun()
else:
