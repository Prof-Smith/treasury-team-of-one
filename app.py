...
st.markdown(f'''<style>
.stApp{{background:{C['bg']}}}.block-container{{padding:.45rem 1.05rem .9rem;max-width:1550px}}[data-testid="stSidebar"]{{background:#081F33}}[data-testid="stSidebar"] *{{color:#F1F7FA!important}}header[data-testid="stHeader"]{{height:1.8rem}}
...
div[data-testid="stMetric"]{{background:white;border:1px solid #D5E1E8;border-radius:11px;padding:.36rem .56rem}}div[data-testid="stMetricLabel"]{{font-size:.68rem}}div[data-testid="stMetricValue"]{{font-size:1.16rem}}.stButton>button{{min-height:34px;border-radius:9px;font-weight:800}}h3{{margin:.25rem 0!important;font-size:1rem!important}}
.clockbar{{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:.65rem;background:white;border:1px solid #D5E1E8;border-radius:10px;padding:.34rem .58rem;margin-bottom:.38rem}}.clocktime{{font-size:.94rem;font-weight:900}}.clocklabel{{font-size:.63rem;letter-spacing:.07em;text-transform:uppercase;color:#687C8E;font-weight:800}}.clocktrack{{height:6px;background:#E5EDF2;border-radius:999px;overflow:hidden}}.clockfill{{height:100%;background:linear-gradient(90deg,#00BAC6,#F4AB00,#D63F42)}}.clockremain{{font-size:.73rem;font-weight:800;color:#D63F42}}
...
def clock(page):
    ...
    st.markdown(f'<div class="clockbar"><div><div class="clocklabel">Briefing clock</div><div class="clocktime">{t}</div></div><div><div class="clocklabel">{phase}</div><div class="clocktrack"><div class="clockfill" style="width:{min(100,e/90*100):.0f}%"></div></div></div><div class="clockremain">{text}</div></div>',unsafe_allow_html=True)
...
if page=='Historical Baseline':
    hero('T−2 | BEFORE MONDAY','Historical forecast performance loaded','The Risk Agent reads the versioned historical-error file automatically.')
    l,r=st.columns(2)
    l.markdown(f'<div class="panel"><div class="eyebrow">AUTOMATIC SOURCE</div><h3>{SAMPLE.name}</h3><div class="row"><span>Status</span><span class="grow"></span><b style="color:#15836D">READY</b></div><div class="row"><span>Observations</span><span class="grow"></span><b>{len(hist)}</b></div></div>',unsafe_allow_html=True)
    r.dataframe(hist.head(8),use_container_width=True,hide_index=True,height=210); jump_button('Build Friday baseline →',1)
